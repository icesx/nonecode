...
2. 设函数 z = f ( x , y ) 在点 $( x_0 , y_0 )$ 具有偏导数,且在点 $( x_0 , y_0)$ 处有极值,则它在该点的偏导数必然为零:f x ( x 0 , y 0 ) = 0 ,f y ( x 0 , y 0 ) = 0
...
2. 损失函数参数的真值为（θ），决策的结果为d ，两者的不一致会带来一定的损失，这种损失是一个随机变量，用L(θ,d)表示。常见的损失函数有
$$L(θ,d) = c(θ − d)^2$$(称为二次损失函数）
### 48 酉矩阵
...
1. 给定初始数据集  $$\\{x_i\\}^N_{i=1}$$  ,K-Means 会把数据分成 K 个簇，每个簇代表不同的类别
